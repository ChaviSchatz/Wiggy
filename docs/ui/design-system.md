...
| `surface-soft` | `#FDFBFD` | board columns and nested panels, to sit *under* `surface` |
...
already *is* 800 weight with tight tracking. The identity step is named `text-identity` rather than
...
- **Border width** is 1px everywhere. A 2px border means exactly one thing: *this is the selected or
  focused item*.
...
- **TopBar** — white, 69px, hairline bottom border, spanning the **main column only**. Carries
  actions: primary `+ New Order` for permitted roles, feedback, profile, sign out. Identity is *not*
