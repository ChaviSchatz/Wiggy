...
- **Task Type** — reusable, standalone catalog item (a _kind_ of work). Holds only its own defaults.
- **Task Group** — a configurable group of task types (e.g. "Color" → Full color, Roots, Highlights).
- **Intake Template** — decides _which_ task types/groups are relevant and _how_ they appear
