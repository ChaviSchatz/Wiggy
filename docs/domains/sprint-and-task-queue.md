...
**Purpose:** An operational layer *on top of* `runtime_tasks` so the production manager can plan a
...
- **Current** task (in progress) → **Next** (top-ranked *available* task) → **rest of queue** by
