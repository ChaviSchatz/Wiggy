...
- **`work_stages`** — ordered production phases (e.g. Intake, Planning, Machine Sewing, Hand Tying,
  ...
  **production board columns**. _Stations are merged into this concept — there is no separate
  stations table._
...
- A **Work Stage** ("Color") = the _phase/where_ work happens (a board column).
- A **Task Group** ("Color") = a _catalog grouping of task types_ (see work-definition domain).
